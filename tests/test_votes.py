...
@pytest.fixture
def test_voted(session,test_posts,test_user):
    voted = models.Votes(post_id=test_posts[3].id,user_id=test_user['id'])
    session.add(voted)
    session.commit()
...
def test_vote_twice_post(authorized_client, test_posts, test_voted):
    res = authorized_client.post(
        "/vote/", json={"post_id": test_posts[3].id, "dir": 1})
